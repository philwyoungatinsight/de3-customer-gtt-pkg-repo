...
locals {
  ...
  # Choose tg-script based on which config key is present — no hardcoded cluster choice.
  # kubeconfig_cli_command set → kubeconfig-from-cli (minikube, kind, OpenShift, etc.)
  # otherwise                 → kubeconfig-from-sops (full kubeconfig stored in SOPS)
